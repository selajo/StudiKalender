...
public:
    ...
    void populateData(const QList<int> &idTermin,const  QList<QString> &Terminname,const QList<QDate> &Datum,const QList<QTime> &Beginn,const QList<QTime> &Ende, const QList<QString> &Beschreibung);
...
inline void TestModel::populateData(const QList<int> &idTermin,const  QList<QString> &Terminname,const QList<QDate> &Datum,const QList<QTime> &Beginn,const QList<QTime> &Ende, const QList <QString> &Beschreibung)
{
    LidTermin.clear();
    LidTermin = idTermin;
    LTerminname.clear();
...
inline QVariant TestModel::data(const QModelIndex &index, int role) const
{
    ...
    if (index.column() == 0) {
        ...
        return LBeschreibung[index.row()];
    }
    return QVariant();
}
