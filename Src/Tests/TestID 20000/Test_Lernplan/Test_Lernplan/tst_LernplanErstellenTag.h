#ifndef TST_LERNPLANERSTELLENTAG_H
#define TST_LERNPLANERSTELLENTAG_H

#include <tst_tst_checkenddatum.h>

//Author: Josef Lanzl

using namespace testing;

//LernplanErstellenTag

TEST(test_20207, test)
{
    {
        //setup
        QSqlDatabase db = QSqlDatabase::addDatabase("QMYSQL");
        db.setHostName("194.95.108.24");
        db.setUserName("root");
        db.setPassword("root");
        db.setDatabaseName("kalender");
        db.open();

        NProfil p;
        if(!p.ProfilCheckNutzername("LernplanTest1")) {
            p.ProfilInsert("LernplanTest1", "Lernen", "lern@mail.de", "l1", "l2");
        }

        p.ProfilLaden("LernplanTest1", "Lernen");
        QDate Start;
        Start.setDate(2018, 8, 16);
        QTime AchtUhr, ZehnUhr, Null;
        AchtUhr.setHMS(8, 0, 0);
        ZehnUhr.setHMS(20, 37, 0);
        Null.setHMS(0, 0, 0);

        Studienfach s;
        int StundenStudienfach = 3;
        if(s.get_idStudienfach("Testen", p.idNutzer) == 0) {
            s.insert_Studienfach("Testen", 5, 4, StundenStudienfach, p.idNutzer);
        }

        Termin t;
        if(!t.TerminCheckVorhanden("Termin", Start, AchtUhr, ZehnUhr,p.idNutzer)) {
            t.TerminInsert("Termin", Start, AchtUhr, ZehnUhr, "Termin", Null, 1, 0, 0, p.idNutzer);
        }


        Lernplan l;
        Kalender k;
        k.TermineInWoche(Start, Start.addDays(6), p.idNutzer);
        k.StudienfachVonNutzer(p.idNutzer);
        l.LernplanErstellenTag(Start,  p.idNutzer);


        AchtUhr = ZehnUhr;
        ZehnUhr.setHMS(22, 0, 0);
        ASSERT_THAT(t.TerminCheckVorhanden("Lernen Testen", Start, AchtUhr, ZehnUhr,p.idNutzer), true);
    }

    //Aufräumen
    QSqlDatabase::removeDatabase("qt_sql_default_connection");
}

TEST(test_20208, test)
{
    {
        //setup
        QSqlDatabase db = QSqlDatabase::addDatabase("QMYSQL");
        db.setHostName("194.95.108.24");
        db.setUserName("root");
        db.setPassword("root");
        db.setDatabaseName("kalender");
        db.open();

        NProfil p;
        if(!p.ProfilCheckNutzername("LernplanTest1")) {
            p.ProfilInsert("LernplanTest1", "Lernen", "lern@mail.de", "l1", "l2");
        }

        p.ProfilLaden("LernplanTest1", "Lernen");
        QDate Start;
        Start.setDate(2018, 8, 17);
        QTime AchtUhr, ZehnUhr, Null;
        AchtUhr.setHMS(8, 0, 0);
        ZehnUhr.setHMS(10, 0, 0);
        Null.setHMS(0, 0, 0);

        Studienfach s;
        int StundenStudienfach = 3;
        if(s.get_idStudienfach("Testen", p.idNutzer) == 0) {
            s.insert_Studienfach("Testen", 5, 4, StundenStudienfach, p.idNutzer);
        }

        Termin t;
        if(!t.TerminCheckVorhanden("Termin", Start, AchtUhr, ZehnUhr,p.idNutzer)) {
            t.TerminInsert("Termin", Start, AchtUhr, ZehnUhr, "Termin", Null, 1, 0, 0, p.idNutzer);
        }


        Lernplan l;
        Kalender k;
        k.TermineInWoche(Start, Start.addDays(6), p.idNutzer);
        k.StudienfachVonNutzer(p.idNutzer);
        l.LernplanErstellenTag(Start,  p.idNutzer);


        AchtUhr = ZehnUhr;
        ZehnUhr.setHMS(12, 0, 0);
        ASSERT_THAT(t.TerminCheckVorhanden("Lernen Testen", Start, AchtUhr, ZehnUhr,p.idNutzer), true);

        AchtUhr = ZehnUhr;
        ZehnUhr.setHMS(13, 0, 0);
        ASSERT_THAT(t.TerminCheckVorhanden("Lernen Testen", Start, AchtUhr, ZehnUhr,p.idNutzer), true);
    }

    //Aufräumen
    QSqlDatabase::removeDatabase("qt_sql_default_connection");
}
#endif // TST_LERNPLANERSTELLENTAG_H
